...
```{r}
Sample_sra <- sf::st_read("./data/SRA_10_10_600.shp", quiet = T) %>% 
  st_set_crs(27700) %>% # The original shp doesn't have crs, so define it as OSGB (27700) 
  st_transform(4326)  # convert CRS to wgs84 (from meter to degree) 
...
Sample_property_sra <-  Sample_property_dist %>% 
...
head(Sample_property_sra)
```


```{r}

Sample_grid_result <- Sample_grid_PV %>% 
  #head() %>% 
  st_join(Sample_property_sra , join = st_intersects) %>% 
  #head()  
  group_by(Grid_ID) %>% 
