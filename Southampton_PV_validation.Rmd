...
uk_grid <- sf::st_read("./data/1km_grid_region.shp") %>% 
...
soton_border <- sf::st_read("./data/Southampton_border.shp") %>% 
...
soton_grid_resample <- raster::rasterize(as(soton_border, "Spatial"), 
                                            raster_base, 
...
leaflet() %>% 
  ...
  #addTiles() %>% 
  addPolygons(data = soton_grid_resample, weight = 1, fillOpacity = 0) %>% 
