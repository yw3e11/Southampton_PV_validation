...
```{r}
temp <- st_transform(soton_grid_resize[1,], 27700)
st_area(temp)
temp$
...
```{r}
plot(soton_grid_resize)
nrow(soton_grid_resize)
```
...
ggplot(soton_grid_residential ) + 
  ...
  geom_text(aes(x=median_soton_grid_res +250 , 
                y=130, 
                label = paste("median of density =",median_soton_grid_res) ), 
                nudge_x = 30 , 
                color ="red" ) + 
  labs(x = "Property density (thousands per square km)" ,  y="Proportion of areas having various density")+
  scale_y_continuous(labels = function(x) paste(round(x/7.46 ,2 ), "%")) +
  scale_x_continuous(labels = function(x) paste(round(x/67868*1000,0)) , breaks = seq(0,1000,200/100000*67868) )
  #ylim(0,150)
...
bins <- quantile(soton_grid_residential$Count, seq(from = 0.1, to =1, by=0.2))
pal <- colorBin("YlOrRd", domain = soton_grid_residential$Count, bins = bins ,  na.color = "#ffffff00")
...
soton_grid_residential%>% 
  ...
  addLegend("bottomright", pal=pal, values = ~Count, title = "Property count" , opacity = 1 , na.label = "empty") %>% 
  addScaleBar() 
...
soton_grid_residential_only %>% 
  ...
  addLegend("bottomright", pal=pal, values = ~Count, title = "Property count" , opacity = 1) %>%   addScaleBar() 
```
...
soton_bld <- sf::st_read("./data/Buildings_grid.shp", quiet = T) %>% 
  st_set_crs(27700) %>% # The original shp doesn't have crs, so define it as OSGB (27700)   
  st_transform(4326)  # convert CRS to wgs84 (from meter to degree) 
...
#                   fillOpacity = 1, fillColor = "red", stroke = 1, color = "red", 
                   popup = ~ADDRESS ) %>% 
  addPolygons(data = soton_bld , fillOpacity = 0, 
...
bld_pv_gis %>% 
  #mutate(NGIS = ifelse(is.na(NGIS), 0, NGIS)) %>% 
  #mutate(AGIS = ifelse(is.na(AGIS), 0, AGIS)) %>% 
  #arrange(NPV, NGIS) %>% 
  #head()
  ggplot()+ 
  geom_point(aes(x= NPV, y=AGIS))
