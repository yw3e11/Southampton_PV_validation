...
```{r}
temp <- read.csv("./sra_kw_result_srf_loop.csv", sep = ",", header = T, stringsAsFactors = F)
...
temp %>% 
  left_join(assign_bid, by = "Toid") %>% 
  dplyr::select( -OBJECTID.. , -Toid , -Shape.., -Shape_Length, -Shape_Area) %>% 
  gather(Parameter, Result, -BID) %>% 
  ...
  summarise(Results = sum(Result)) %>% 
  #head()
  plot_ly(x=~Curvature, y=~focalSum, z=~Results, color=~as.factor(Radiation)) 
  ...
  #  #geom_line(aes(Curvature, Result, group=focalSum, color = as.factor(focalSum)))
  #  geom_contour(aes(Curvature, focalSum, z=Result, color=as.factor(focalSum)))
  #head()
  #head()
  ...
  #ggplot()+
    #group_by(Parameter)+
    #facet_grid(~focalSum)+
    #geom_line(aes(x=Value, y=Result, group=Parameter , color = Parameter))
    #geom_contour(aes(Curvature, Radiation, z= Result))
