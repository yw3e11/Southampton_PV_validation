...
fig_soton_grid_bld_density <- soton_grid_bld_density %>% 
  ggplot()+
  geom_histogram(aes(x=bld_density), bins = 12)

fig_grid_sample_bld <- soton_grid_sample %>% 
  ggplot()+
...
soton_grid_sample %>% 
  ...
  leaflet() %>% 
    addTiles() %>% 
    addPolygons(popup = ~as.character(bld_density)) %>% 
    addPolygons(data = soton_residential_bld)
...
temp1 %>% 
  ...
  leaflet() %>% 
    addTiles() %>% 
    addMarkers(popup = ~as.character(Properties_per_bld)) %>% 
    addPolygons(data=temp1)
```
